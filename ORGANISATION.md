...
Dans le workspace organisé, ouvrir **`actifs/fireviewer/fireviewer-contracts`**. Chaque dossier est un dépôt Git autonome. Créer une branche de chantier dans ce dépôt ; pour un travail parallèle, créer un worktree sous `travaux/<chantier>/<composant>`. Ne jamais commiter depuis la racine multi-dépôts, `archives/` ou un ancien chemin de compatibilité.

Schémas et interfaces partagés entre les composants FV ; compatibilité des identifiants et formats.

Pas de calcul producteur, persistance, UI ou copie locale des contrats dans chaque consommateur.
...
Version de package déclarée dans les sources : **0.1.3**. Version candidate de la migration par révisions. Les tags et artefacts reçus restent immuables ; un commit de cette branche ne déploie pas le service.
